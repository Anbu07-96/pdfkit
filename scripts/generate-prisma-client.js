...
function isRealGeneratedClient() {
  // The real generated client is identified by its "code generated by
  // Prisma" header AND by loading successfully. A failed/interrupted
  // generation (e.g. @prisma/client's own postinstall dying mid-run in a
  // network-restricted environment) leaves ~2KB shim files behind that
  // throw "did not initialize yet" at use, and the development stub (written
  // below) has neither the header nor PrismaClient-compatible behavior —
  // file-existence alone cannot distinguish these states (Phase 65
  // hardening, both states observed in fresh sandbox installs).
  try {
    const source = fs.readFileSync(realIndex, 'utf8');
    if (!source.includes('code generated by Prisma')) return false;
    const mod = require(realIndex);
...
if (fs.existsSync(realIndex)) {
  if (isRealGeneratedClient()) {
    // A real `prisma generate` output is present — keep it, and remove any
    // stale stub marker left over from an earlier stub (the marker must not
    // outlive the real client: a later npm install would otherwise see the
    // marker and overwrite the real client with the stub again).
    if (fs.existsSync(stubMarkerFile)) fs.rmSync(stubMarkerFile);
    console.log(
  ...
  console.log(
    'Prisma client files present but not a real generated client (interrupted generation) — replacing with the development stub.',
  );
}
