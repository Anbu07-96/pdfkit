...
export const HOME_FAQ: FaqItem[] = [
  {
    ...
    answer:
      "PDFKit is a web app for everyday PDF and document tasks — organising pages, converting between formats, editing, securing and, later, OCR and AI document tools. It is in early development: Merge PDF works today and the remaining tools in the catalog are being built one at a time.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "For a working tool such as Merge PDF, your files are sent to the PDFKit server over HTTPS, held in memory only while the operation runs, and discarded as soon as the result is returned. They are never written to disk, never stored and never logged — only counts and timings are recorded. Tools that are not implemented upload nothing at all.",
  },
  {
    question: "What tools does PDFKit support?",
    answer: (
      <>
        The catalog lists {TOOLS.length} tools across six categories. Merge PDF is
        implemented and free to use; every other tool page states its availability
        clearly instead of pretending to work. You can{" "}
