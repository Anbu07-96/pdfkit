...
import {
  type ExtractImagesPageMode,
} from "@/lib/processing/extract-images";
import { formatBytes } from "@/lib/utils/format";
import { cn } from "@/lib/utils/cn";

export interface ExtractImagesWorkspaceProps {
...
const PAGE_OPTIONS: { value: ExtractImagesPageMode; title: string; description: string }[] = [
  { value: "all", title: "All pages", description: "Extract images from every page." },
  { value: "first", title: "First page", description: "Extract images from page 1 only." },
  { value: "last", title: "Last page", description: "Extract images from the final page only." },
];
...
export function ExtractImagesWorkspace({ limits }: ExtractImagesWorkspaceProps) {
  const [files, setFiles] = React.useState<SelectedFile[]>([]);
  const [pageCount, setPageCount] = React.useState<number | null>(null);
  const [pages, setPages] = React.useState<ExtractImagesPageMode>("all");
  ...
  async function handleExtract() {
    ...
    try {
      const document = await runExtractImages({
        file: file.file,
        pages,
      ...
      showToast({
        ...
        description: `Extracted ${document.artifacts} image${
          document.artifacts === 1 ? "" : "s"
        }.`,
      });
    } catch (error) {
      ...
      setFailure(toFailure(error, "Extraction failed."));
      setStatus("error");
    } finally {
      abortRef.current = null;
    }
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Button size="lg" onClick={handleExtract} disabled={!canRun}>
          {busy ? (
            <Loader2 aria-hidden="true" className="size-4 animate-spin" />
          ) : (
            <ImageIcon aria-hidden="true" className="size-4" />
          )}
          {busy ? "Extracting images…" : "Extract Images"}
        </Button>
