...
import {
  type PdfToTextPageMode,
} from "@/lib/processing/pdf-to-text";
import { formatBytes } from "@/lib/utils/format";
import { cn } from "@/lib/utils/cn";

export interface PdfToTextWorkspaceProps {
...
const PAGE_OPTIONS: { value: PdfToTextPageMode; title: string; description: string }[] = [
  { value: "all", title: "All pages", description: "Extract text from every page." },
  { value: "first", title: "First page", description: "Extract text from page 1 only." },
  { value: "last", title: "Last page", description: "Extract text from the final page only." },
];
...
export function PdfToTextWorkspace({ limits }: PdfToTextWorkspaceProps) {
  const [files, setFiles] = React.useState<SelectedFile[]>([]);
  const [pageCount, setPageCount] = React.useState<number | null>(null);
  const [pages, setPages] = React.useState<PdfToTextPageMode>("all");
  ...
  async function handleConvert() {
    ...
    try {
      const document = await runPdfToText({
        file: file.file,
        pages,
      ...
      setFailure(toFailure(error, "Text extraction failed."));
      setStatus("error");
    } finally {
      abortRef.current = null;
    }
  ...
  return (
    <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="rounded-xl border border-border bg-surface p-4">
            <h3 className="inline-flex items-center gap-2 text-sm font-medium text-foreground">
              ...
              Plain text extraction, honestly described
            </h3>
            <p className="mt-2 text-sm text-muted">
              Extracts plain text contained in the PDF stream, preserving page boundaries.
              Scanned or image-only pages contain no extractable text — OCR is NOT performed.
            </p>
          </div>
        </div>
      ) : null}
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Button size="lg" onClick={handleConvert} disabled={!canRun}>
          {busy ? (
            <Loader2 aria-hidden="true" className="size-4 animate-spin" />
          ) : (
            <FileText aria-hidden="true" className="size-4" />
          )}
          {busy ? "Extracting text…" : "Convert to Text"}
        </Button>
