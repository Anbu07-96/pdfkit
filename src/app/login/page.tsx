...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res?.error) {
        setError("Invalid email address, disposable domain, or incorrect password.");
      } else if (res?.url) {
  ...
  return (
    <main className="container max-w-md py-12">
      <SectionHeader
        ...
        description="Access your account, plan quotas and document security settings."
      />

      <div className="mt-8 rounded-2xl border border-border bg-surface p-6 shadow-xs">
        {/* Prominent OAuth Authentication Providers */}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <Input
            ...
            required
            autoComplete="email"
            hint="Must be a valid non-disposable email address."
          />
